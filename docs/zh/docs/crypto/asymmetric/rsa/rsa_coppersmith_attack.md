...
最直接的Coppersmith用法（求解方程小根）

```
sage: R.<m> = PolynomialRing(Zmod(n))                                                          
...
既然他们互素，那么我们可以根据**中国剩余定理**，可得$m^3 \equiv C \bmod n_1n_2n_3$。
...
短字节填充攻击，其中要用到 Coppersmith 和 Franklin-Reiter相关消息攻击

场景如下：

已知公钥 (n, e)，**明文为M**，在明文M的末尾填充随机字节，一种实现方法是
...
这里的 $g_1,g_2$ 都是定义在模 $n$ 的多项式环上的.

当 $x = m_1,y =  r_2-r_1$ 时，

$$
g_1 = m_1^e-c_1=0\\
g_2=(m_1+r_2-r_1)^e-c_2=m_2^e-c_2=0
$$

即 $g_1,g_2$ 有公共的根 $x=m_1$.

设 $h(y) = res_x(g_1, g_2)$，这里的 $res_x$ 表示多项式$g_1,g_2$的结式（resultant）.

我们略过它的定义（反正在sage中可以直接求得），利用结式的性质：多项式$f(x), g(x)$ 有公共根的充要条件是结式$R(f,g)=0$.

令 $h(y)=0$，可以用 Coppersmith 求解方程得到 $y$. 

求得 $y$ 后，再用Franklin-Reiter相关消息攻击即可.
...
这里的M是`flag = flag + os.urandom(8)`

变形的方法是将`m1`左移64位，将`r1`看做0，经过这样的处理后，
...
这里的`r = r2 = gmpy2.s2n(os.urandom(8))`

再经过加密，

$$
\begin{aligned}
newc_1&=newm_1^e\mod n\\
&=(2^{64}m_1)^e \mod n\\
&=2^{64e}c_1 \mod n
\end{aligned}
$$

因此，我们可以通过`c1`求出`new_c1`，问题就转化为普通的Short-Pad Attack了.
